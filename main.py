...
def analyze(stocks: list[dict], close: pd.DataFrame, volume: pd.DataFrame) -> dict:
    ...
    for ticker, ret in returns.items():
        ...
        sector_stocks.setdefault(sector, []).append(stock_info(ticker, ret))
    sector_avg = {s: round(sum(v) / len(v), 2) for s, v in sector_buckets.items()}

    def _industry_entry(sector: str, avg: float) -> dict:
        stocks_sorted = sorted(sector_stocks[sector], key=lambda x: -x["value"])
        return {
            "sector": sector,
            "avg_return": avg,
            "top_gainers": stocks_sorted[:3],
            "top_losers": stocks_sorted[-3:][::-1] if len(stocks_sorted) > 1 else [],
        }

    sorted_sectors = sorted(sector_avg.items(), key=lambda x: x[1])
    industry_movers = {
        "positive": [_industry_entry(s, a) for s, a in sorted_sectors[-3:][::-1]],
        "negative": [_industry_entry(s, a) for s, a in sorted_sectors[:3]],
    }
    ...
    return {
        ...
        "industry_movers": industry_movers,
    }
...
def _render_industry_group(industries: list[dict]) -> str:
    blocks = []
    for ind in industries:
        sign = "+" if ind["avg_return"] >= 0 else ""
        block = f"#### {ind['sector']} | avg {sign}{ind['avg_return']:.2f}%\n"
        if ind["top_gainers"]:
            rows = [{"Stock": s["name"], "Ticker": s["ticker"], "5D Change": f"{s['value']:+.2f}%"}
                    for s in ind["top_gainers"]]
            block += "\n**Top movers (positive)**\n\n"
            block += _md_table(rows, ["Stock", "Ticker", "5D Change"], ["Stock", "Ticker", "5D Change"])
        if ind["top_losers"]:
            rows = [{"Stock": s["name"], "Ticker": s["ticker"], "5D Change": f"{s['value']:+.2f}%"}
                    for s in ind["top_losers"]]
            block += "\n\n**Top movers (negative)**\n\n"
            block += _md_table(rows, ["Stock", "Ticker", "5D Change"], ["Stock", "Ticker", "5D Change"])
        blocks.append(block)
    return "\n\n".join(blocks)
...
{_render_industry_group(analysis['industry_movers']['positive'])}

### Worst Performing Industries

{_render_industry_group(analysis['industry_movers']['negative'])}
